...
public class PanelDsThanhVien extends JPanel implements ActionListener {
	...
	public PanelDsThanhVien() {
		...
		String row[] = { "M\u00E3 th\u1EBB", "T\u00EAn kh\u00E1ch h\u00E0ng", "S\u1ED1 \u0111i\u1EC7n tho\u1EA1i",
				"Lo\u1EA1i th\u1EBB", "S\u1ED1 \u0111i\u1EC3m", "Ng\u00E0y \u0111\u0103ng k\u00FD" };
...
//						// Tìm theo SDT
				...
				list1.forEach(i -> {
					...
					model.addRow(new Object[] { i.getMaTTV(), i.getKhachHang().getTenKH(), i.getKhachHang().getSdt(),
							i.getLoaiThe().getTenLoaiTTV(), i.getDiemTichLuy(), i.getNgayDangKy() });
				});
			}
		...
		if (o.equals(btnXoa)) {
			...
			if (true) {
				...
				if (selectedDate != null) {
					ngaytao = selectedDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
				} else {
				...
				TheThanhVien ttv = new TheThanhVien(mattv, new KhachHang(tenttv, sdt), new LoaiThe(loaiThe, null),
						soDiem, ngaytao);
	...
	public void docDL() {
		...
		listTTV.forEach(e -> {
			model.addRow(new Object[] { e.getMaTTV(), e.getKhachHang().getTenKH(), e.getKhachHang().getSdt(),
					e.getLoaiThe().getTenLoaiTTV(), e.getDiemTichLuy(), e.getNgayDangKy() });
		});
	...
	public void refreshTableData() {
		...
		for (TheThanhVien ttv : newData) {
			Object[] row = { ttv.getMaTTV(), ttv.getKhachHang().getTenKH(), ttv.getKhachHang().getSdt(),
					ttv.getLoaiThe().getTenLoaiTTV(), ttv.getDiemTichLuy(), ttv.getNgayDangKy() };
			model.addRow(row);
		}
