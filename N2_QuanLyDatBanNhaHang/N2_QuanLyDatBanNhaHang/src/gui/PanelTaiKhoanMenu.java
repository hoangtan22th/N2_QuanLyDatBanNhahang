...
public class PanelTaiKhoanMenu extends JPanel implements ActionListener {
	...
	public PanelTaiKhoanMenu() {
		...
		ImageIcon calendarIcon = new ImageIcon(
				"D:\\Data\\tailieuvuive\\PTUD\\N2_QuanLyDatBanNhahang\\N2_QuanLyDatBanNhaHang\\img\\lich.png");
		...
		txtTim.addKeyListener((KeyListener) new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				list1.forEach(i -> {
					...
					modalTable.addRow(new Object[] { i.getTaiKhoan(), i.getMatKhau(),
							i.isRole() ? "Quản lý" : "Nhân viên", i.getNgayTaoTK()
	...
	public void docDL() {
		...
		listTK.forEach(e -> {
			modalTable.addRow(new Object[] { e.getTaiKhoan(), e.getMatKhau(), e.isRole() ? "Quản lý" : "Nhân viên",
					e.getNgayTaoTK() });
	...
	public void refreshTableData() {
		...
		for (TaiKhoan tk : newData) {
			Object[] row = { tk.getTaiKhoan(), tk.getMatKhau(), tk.isRole() ? "Quản lý" : "Nhân viên",
					tk.getNgayTaoTK() };
			model.addRow(row);
		}
